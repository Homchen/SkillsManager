/**
 * Claude Code hook: record skill usage.
 *
 * Covers:
 * - UserPromptExpansion: user typed /skill-name (does NOT go through Read)
 * - PreToolUse Skill: Claude invokes the Skill tool
 * - PreToolUse Read: agent reads a SKILL.md file
 *
 * Shares ~/.skillsmanager/skills/skill-usage.json with the Cursor hook.
 * Always exit 0 with empty stdout (UserPromptExpansion stdout becomes model context).
 */
...
async function recordSkillUsage(skillName, filePath) {
  ...
  const key = String(skillName).trim().replace(/^\//, "");
...
async function main() {
  try {
    ...
    if (eventName === "UserPromptExpansion") {
      const commandName = typeof payload.command_name === "string" ? payload.command_name.trim() : "";
      if (!commandName) {
        return;
      }
      await recordSkillUsage(commandName, undefined);
      return;
    }
